...
var vr_y_high = 2.158;
var vr_width  = -1.5984;
var vr_height = 2.1378;

var player_positions = {};
var player_vr_positions = {};
var vr_info = {
    blockers: []
};

function convert_to_vr_coord(xy){
    var x = ((xy[0] - 3)/map_json.width)*vr_width + vr_x_low;
    var y = ((xy[1] - 3)/map_json.height)*vr_height + vr_y_low;
    return [x, y];
}
...
function init(){
	...
	var index = 1; // Math.min(lobby.length - 1, 7);
	player_positions[socket.name] = map_json.spawn[index];
	player_vr_positions[socket.name] = convert_to_vr_coord(map_json.spawn[index]);
	...
	socket.on('prep', function(){
	    ...
	    setTimeout(function(){socket.emit("game_start")}, 1000);
	});

	socket.on('player_update', function(coord){
	    coord = JSON.parse(coord);
	    var box0 = new SAT.Box(new V(coord[0], coord[1]), 10, 10).toPolygon();

	    var response = new SAT.Response();

	    var collided = false;

	    for(var i=0; i<vr_info.blockers.length; ++i){
		if(SAT.testPolygonPolygon(box0, vr_info.blockers[i], response)){
		    collided = true;
		    console.log("Player-blocker collision!");
		    return;
		}
	    }
		...
		if(key != socket.name){
		    ...
		    var box1 =  new SAT.Box(new V(pcoord[0], pcoord[1]), 10, 10).toPolygon();
		...
		player_vr_positions[socket.name] = convert_to_vr_coord(coord);
	    }
	});
	...
	socket.on('vr_start', function(){
	    state = "game";
	    player_nsp.emit("game_start");
	    setTimeout(game_update, 100);
	});

	socket.on('vr_blocker_update', function(data){
	    console.log("blockers update", data);
		...
		var arr = [];
		var rad = data[bi].radius;
		var coord = data[bi].coord;
		var x = ((coord[0] - vr_x_low) / vr_width) * map_json.width;
		var y = ((coord[1] - vr_y_low) / vr_height) * map_json.height;
		
		vr_info.blockers.push(new SAT.Circle(new SAT.Vector(x, y), rad));
	    }
